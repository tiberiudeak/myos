...
int main(int argc, char *argv[]) {
	...
	display_menu(win1, highlight, main_menu, n_choices, MAIN_MENU_X,
				 MAIN_MENU_Y);
	keypad(win1, TRUE);

	// display info message for the highlighted choice in win1
	display_message(win2, main_menu[highlight - 1].help, SEC_MENU_X, SEC_MENU_Y,
					"Info");
	...
	while (1) {
		...
		display_menu(win1, highlight, main_menu, n_choices, MAIN_MENU_X,
					 MAIN_MENU_Y);
		display_message(win2, main_menu[highlight - 1].help, SEC_MENU_X,
						SEC_MENU_Y, "Info");
	}
...
void display_message2(WINDOW *win, struct Config config, int x, int y,
					  char *extra) {
	if (config.type == BOOL && config.default_val == 1) {
		...
		if (extra) {
			mvwprintw(win, y, x, "(%d) %s %s", config.default_val,
					  config.prompt, extra);
		} else {
...
void display_configs(WINDOW *win, int highlight, struct Config *configs,
					 int n_configs, int n_choices, int x, int y) {
...
void display_choices(WINDOW *win, int highlight, struct Choice *choices,
...
void display_menu(WINDOW *win, int highlight, struct Menu *entries,
				  int n_entries, int x, int y) {
...
void display_choice(WINDOW *win, struct Choice choice, WINDOW *win2) {
	...
	display_configs(choice_win, highlight, choice.configs, choice.n_configs, 0,
					SEC_MENU_X, SEC_MENU_Y);

	display_message(win2, choice.configs[0].help_message, SEC_MENU_X,
					SEC_MENU_Y, "Info");
	...
	while (1) {
		...
		display_configs(choice_win, highlight, choice.configs, choice.n_configs,
						0, SEC_MENU_X, SEC_MENU_Y);
		display_message(win2, choice.configs[highlight - 1].help_message,
...
void display_int_window(WINDOW *win, struct Config *config) {
	...
	display_message(int_win, "Insert number: ", SEC_MENU_X, SEC_MENU_Y,
					config->prompt);
	...
	while (1) {
		...
		if (ch != 10) {
			if (ch >= '0' && ch <= '9') {
				...
				// TODO: store characters in a buffer and convert it later to
				// int
				buffer[index++] = ch;
			}
		} else {
...
void display_submenu(WINDOW *win, struct Menu menu, WINDOW *win2) {
	...
	if (menu.choices != NULL) {
		...
		display_message(win2, menu.choices[0].help_message, SEC_MENU_X,
						SEC_MENU_Y, "Info");
	}

	if (menu.configs != NULL && menu.choices != NULL) {
		display_configs(new_window, highlight, menu.configs, menu.n_configs,
						menu.n_choices, MAIN_MENU_X,
						menu.n_choices + MAIN_MENU_Y);
	} else {
		display_configs(new_window, highlight, menu.configs, 0, menu.n_configs,
						MAIN_MENU_X, MAIN_MENU_Y);
		display_message(win2, menu.configs[0].help_message, SEC_MENU_X,
						SEC_MENU_Y, "Info");
	}
	...
	while (1) {
		...
		case 121:
			...
			if (highlight > menu.n_choices &&
				check_dependencies(
					menu.configs[highlight - 1 - menu.n_choices])) {
		...
		case 110:
			...
			if (highlight > menu.n_choices &&
				...
				if (strcmp(menu.configs[highlight - 1 - menu.n_choices].prompt,
						   "Done") != 0) {
					menu.configs[highlight - 1 - menu.n_choices].default_val =
						0;
				}
#else
		...
		if (choice != 0) {
			...
			if (choice <= menu.n_choices) {
				...
				if (check_dependencies(
						menu.configs[choice - 1 - menu.n_choices])) {
					display_int_window(
						win, &menu.configs[choice - 1 - menu.n_choices]);
				}
			}
		...
		if (highlight <= menu.n_choices) {
			...
			display_message(
				win2, menu.configs[highlight - 1 - menu.n_choices].help_message,
				SEC_MENU_X, SEC_MENU_Y, "Info");
		}
	}
}
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < current.n_choices; j++) {
			...
			for (int k = 0; k < c.n_configs; k++) {
				if (check_dependencies(c.configs[k])) {
					if (c.configs[k].default_val == 1 &&
						c.configs[k].type == BOOL) {
						fprintf(fp, "%s=y\n", c.configs[k].symbol);
						printf("%s=y\n", c.configs[k].symbol);
					} else if (c.configs[k].type == INT) {
						fprintf(fp, "%s=%d\n", c.configs[k].symbol,
								c.configs[k].default_val);
						printf("%s=%d\n", c.configs[k].symbol,
							   c.configs[k].default_val);
					}
				}
		...
		for (int j = 0; j < current.n_configs; j++) {
			if (check_dependencies(current.configs[j])) {
				if (current.configs[j].default_val == 1 &&
					current.configs[j].type == BOOL) {
					fprintf(fp, "%s=y\n", current.configs[j].symbol);
					printf("%s=y\n", current.configs[j].symbol);
				} else if (current.configs[j].type == INT) {
					fprintf(fp, "%s=%d\n", current.configs[j].symbol,
							current.configs[j].default_val);
					printf("%s=%d\n", current.configs[j].symbol,
						   current.configs[j].default_val);
				}
			}
