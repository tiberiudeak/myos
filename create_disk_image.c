...
/**
 * @brief Get number of bytes needed for padding until the given limit is
 * reached
 *
 ...
 *
 * This function writes the first block in the disk image. Currently, it writes
 * the first and second sectors (that make the entire bootloader) and adds
...
int write_boot_block(struct file_pointer_type files[], FILE *image_fp) {
	...
	int ret =
		fread((void *) boot_block.sectors[0], FS_SECTOR_SIZE, 1, files[0].fp);
 ...
 *
 * This function writes the superblock to the final disk image. For more details
 * about the information in the superblock, see comments below.
 *
 ...
 * @return 1 if error occured, 0 otherwise
 */
int write_superblock(struct superblock *superblock,
					 struct file_pointer_type files[], FILE *image_fp,
					 int num_files, int total_file_blocks) {
	...
	// set number of blocks for inode bitmap to 1 (TODO: this will have to
	// change)
	...
	superblock->first_data_bitmap_block =
		superblock->first_inode_bitmap_block + superblock->inode_bitmap_blocks;

	// get total disk size, then see to how many blocks it converts to (the
	// number of blocks will be the number of bits in the bitmap)
	uint32_t disk_size = get_disk_size(files, num_files);
	uint32_t data_blocks =
		bytes_to_blocks(disk_size) + 1; // +1 for the root directory block
	superblock->data_bitmap_blocks =
		data_blocks / (FS_BLOCK_SIZE * 8) +
		((data_blocks % (FS_BLOCK_SIZE * 8) > 0) ? 1 : 0);

	superblock->inode_blocks =
		bytes_to_blocks(superblock->total_inodes * sizeof(struct inode_block));

	superblock->first_inode_block =
		superblock->first_data_bitmap_block + superblock->data_bitmap_blocks;
	superblock->first_data_block =
		superblock->first_inode_block + superblock->inode_blocks;
	...
	superblock->first_free_inode_bit =
		superblock->inode_blocks; // used when creating a file
	superblock->first_free_data_bit =
		total_file_blocks + 1; // used when allocating data
 ...
 *
 * This function writes the inodes in the final image. The first inode is the
 * reserved inode and will be all zeroes, the second one describes the root
 * directory, the third one will be the kernel and the rest the remaining files.
 *
 * @param image_fp		File pointer to the disk image
 ...
 * @return 1 if error occured, 0 otherwise
 */
int write_inodes(FILE *image_fp, int num_files, struct superblock *superblock,
				 struct file_pointer_type files[]) {
	uint32_t written_bytes = 0;
	...
	inode.size_bytes = sizeof(struct directory_entry) *
					   (num_files - 1); // -1 for the bootloader
	inode.size_sectors = bytes_to_sectors(inode.size_bytes);

	inode.extent[0] =
		(struct extent_block) {.first_block = superblock->first_data_block,
	...
	uint32_t id = 1;
	uint32_t current_file_first_block =
		superblock->first_data_block + inode.extent[0].length;
	...
	for (size_t i = 1; i < num_files; i++) {
		...
		inode.extent[0] =
			(struct extent_block) {.first_block = current_file_first_block,
	...
	uint32_t written_inodes =
		num_files + 1; // -1 + 2 (minus bootloader, add root and reserved)
	uint8_t zero[FS_BLOCK_SIZE] = {0};
	uint32_t padding = padding_bytes(written_bytes, FS_BLOCK_SIZE);
 ...
 *
 * This function writes the data blocks to the disk image. The first data block
 * contains the root directory's data (one directory entry for each file, as
 * well as for . and ..). Starting with the second data block is the kernel
 * data. After that, data for the remaining files.
 *
 ...
 * @return 1 if error occured, 0 otherwise
 */
int write_data(FILE *image_pt, int num_files, struct superblock *superblock,
			   struct file_pointer_type files[]) {
	uint32_t written_bytes = 0;

	// write first data block which is the root directory that will contain a
	// directory entry for each file/dir
...
int main(int argc, char *argv[]) {
	...
	for (uint32_t i = 0; i < num_files; i++) {
		...
		if (i == num_files - 1 && found != 2) {
			printf("Error: bootloader or kernel not found in bin/ directory! "
				   "Names should be: bootloader and kernel\n");
			return 1;
		}
	}
	...
	ret = write_superblock(&superblock, files, image_fp, num_files,
						   total_file_blocks);
