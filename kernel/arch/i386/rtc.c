...
uint8_t cmos_update_in_progress(void) {
	...
	// read byte from data port and check if update in progress bit is set (bit
	// 7)
...
void rtc_print_datetime(void) {
	...
	do {
		...
		time1.month = cmos_get_rtc_register(CMOS_RTC_REG_MONTH);
		time1.year = cmos_get_rtc_register(CMOS_RTC_REG_YEAR);
	} while ((time1.seconds != time2.seconds) ||
			 (time1.minutes != time2.minutes) || (time1.hour != time2.hour) ||
			 (time1.day != time2.day) || (time1.month != time2.month) ||
	...
	if (!(regB_value & CMOS_RTC_REG_B_BINARY)) {
		...
		time1.hour = ((time1.hour & 0x0F) + (((time1.hour & 0x70) >> 4) * 10)) |
					 (time1.hour & 0x80);
...
// real time updates (so nothing will be printed to the bottom row except the
// datetime)
